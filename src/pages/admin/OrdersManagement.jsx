...
const OrdersManagement = () => {
  ...
  const handleDeleteOrder = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${baseurl}/api/orders/${selectedOrder.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  // Get customer name (for both registered users and guests)
  const getCustomerName = (order) => {
    return order.user?.name || order.customer_name || 'N/A';
  };

  // Get customer email (for both registered users and guests)
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            <p><strong>Name:</strong> {selectedOrder.customer_name || selectedOrder.user?.name || 'N/A'}</p>
                            <p><strong>Email:</strong> {selectedOrder.user?.email || 'N/A'}</p>
                            <p><strong>Phone:</strong> {selectedOrder.customer_phone || 'N/A'}</p>
                            <p><strong>User Type:</strong> {selectedOrder.user_id ? 'Registered User' : 'Guest Customer'}</p>
                                    ...
                                    <tr key={item.id}>
                                      <td className="px-4 py-3">
                                        <div className="flex items-center">
                                          <img
                                            src={item.product?.image ? `${baseurl}/${item.product.image}` : "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&q=80"}
                                          ...
                                          <div>
                                            ...
                                            {item.product?.ingredients && (
                                              <p className="text-xs text-gray-500">
                                                {JSON.parse(item.product.ingredients).join(', ')}
                                              </p>
                                            )}
                                          </div>
                                        </div>
